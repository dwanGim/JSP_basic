...
<%
	...
	for (int i = 0; i < cookies.length; i++) {
		...
		// 쿠키값을 얻어와서 콘솔에 찍어보세요.
		//String coValue = cookies[i].getValue();
		//System.out.println(coValue);
		// 땅콩 쿠키가 존재하는지 체크해서 있으면 땅콩쿠키가 존재합니다.
		// 없으면 존재하지 않습니다 라고 출력해주는 코드.
		if(str.equals("peanut_cookie")) {
			out.println("땅콩맛쿠키가 존재합니다. <br/>");
			flag = true;
			}
			
		}
		// flag가 true로 바뀌었다는 건 땅콩쿠키가 존재한다는 것.
		// 바꿔 말하면 바뀌지 않았다는 것은 땅콩쿠키가 없다는 것
		if(!flag) {
			out.println("땅콩쿠키가 없어졌습니다.");
	}
